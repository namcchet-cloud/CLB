/**
 * Raven Lin - reference-photo Driver rig, v6.1.1.
...
const ATLAS = new URL('../../assets/raven/driver-parts-v611.webp', import.meta.url).href;
const DURATION = 5500;
...
export function initRaven() {
  ...
  const motionEnabled = () => root.dataset.motion !== 'off' && window.ClubMotion?.enabled !== false;
  // The separate replay button grants one-time consent, never a saved override.
  const allowed = (run = active) => !document.hidden && (run?.explicit === true || motionEnabled());
 ...
 <path d="M191 232H208V313H191ZM758 232H775V313H758Z" fill="url(#rv-metal-v61)" stroke="#afadb9" stroke-width="2"/>
  ...
  function runTimeline(run) {
    ...
    const soft = root.dataset.motionChoice === 'quiet' || !motionEnabled();
  ...
  async function launch(trigger, { explicit = false } = {}) {
    ...
    if (typeof Element.prototype.animate !== 'function') {
      lastError = 'This browser does not support the Driver animation';
      return false;
    }
    const words=WORDS[root.lang==='en'?'en':'vi'];
    const layer=makeLayer(words);
    const run={layer,trigger:trigger || document.activeElement,animations:[],audio:null,master:null,origin:null,explicit};
    if (explicit) layer.dataset.userRequested = 'true';
  ...
  document.addEventListener('club:motion',()=>{if(!motionEnabled())stop('motion-off');});
  ...
  const api={launch,stop,preload,get state(){return {version:'6.1.1',active:!!active,phase:active?phase(active):'idle',elapsed:elapsed(active),duration:DURATION,sound:!!active?.audio,lastError};}};
