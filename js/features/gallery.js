import { local, t, apply } from '../core/i18n.js?v=7.0.3';
import { setImage, imageVariant } from '../core/images.js?v=7.0.3';
import { start, loadCSS, report } from '../core/runtime.js?v=7.0.3';
...
export function initGallery(content) {
  ...
  const effectsCSS = new URL('../../css/effects.css?v=7.0.3', import.meta.url).href;
  let cssReady;
  ...
  async function effect(replay = false) {
    ...
    try {
      ...
      if (selected === 'akiko-oishi' && replay && motion?.enabled) {
        const akiko = await start('akiko', async () => (await import('./akiko.js?v=7.0.3')).initAkiko());
        if (rev === effectRevision && artistId === selected) await akiko.launch(btn);
      }
      if (selected === 'raven-lin' && replay && motion?.enabled) {
        const raven = await start('raven', async () => (await import('./raven.js?v=7.0.3')).initRaven());
