import { local, t, apply } from '../core/i18n.js?v=5.1.0';
import { setImage, imageVariant } from '../core/images.js?v=5.1.0';
import { start, loadCSS, report } from '../core/runtime.js?v=5.1.0';
...
export function initGallery(content) {
  ...
  const effectsCSS = new URL('../../css/effects.css?v=5.1.0', import.meta.url).href;
  let cssReady;
  ...
  async function effect(replay = false) {
    ...
    try {
      ...
      if (artistId === 'akiko-oishi' && replay && motion?.enabled) {
        const akiko = await start('akiko', async () => (await import('./akiko.js?v=5.1.0')).initAkiko());
