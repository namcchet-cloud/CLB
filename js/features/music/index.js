import { createMotor } from './motor.js?v=7.0.11';
import { createSpotify } from './spotify.js?v=7.0.11';
import { local } from '../../core/i18n.js?v=7.0.11';
import { setImage } from '../../core/images.js?v=7.0.11';
...
export function initMusic(content) {
  ...
  function beginDrag(event, id) {
    ...
    // Touch/drag itself is an early intent signal. Make sure the selected entity
    // is already being prepared while the listener moves the physical record.
    transport.prepare(record(id));
  ...
  function togglePlay() {
    ...
    if (!transport.ready || !transport.controllable) {
      ...
      transport.ensure(record(id)).then(() => {
        if (id !== state.loadedId || state.phase !== 'ON_TURNTABLE') return;
        setStatus(transport.ready ? 'readyAgain' : transport.fallback ? 'fallback' : 'connecting');
        sync();
      }).catch(error => {
    ...
    // The deck responds to the user's click immediately. Spotify events then
    // confirm/adjust the state, but the platter can never remain frozen because
    // an observer missed a frame.
    motor.setPlaying(wanted);
    motor.forceWake?.();
