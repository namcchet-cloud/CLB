import { local, t, apply } from '../core/i18n.js?v=6.1.0';
import { setImage, imageVariant } from '../core/images.js?v=6.1.0';
import { start, loadCSS, report } from '../core/runtime.js?v=6.1.0';
...
export function initGallery(content) {
  ...
  const effectsCSS = new URL('../../css/effects.css?v=6.1.0', import.meta.url).href;
  let cssReady;
  const ensureEffectsCSS = () => cssReady ||= loadCSS(effectsCSS).catch(e => { cssReady = null; throw e; });
  const ensureRaven = () => start('raven', async () => (await import('./raven.js?v=6.1.0')).initRaven());
  async function effect(replay = false) {
    ...
    try {
      ...
      if (artistId === 'akiko-oishi' && replay && motion?.enabled) {
        const akiko = await start('akiko', async () => (await import('./akiko.js?v=6.1.0')).initAkiko());
  ...
  function render() {
    ...
    if (!items.length) {
      if (artistId === 'raven-lin' && !artworks.some(a => a.artistId === artistId)) {
        ...
        replay.disabled = motion?.enabled === false;
        copy.append(replay);
        const figure = make('figure', 'raven-empty-driver');
        const driver = make('img', '');
        driver.src = new URL('../../assets/raven/driver-reference-v61.webp', import.meta.url).href;
  ...
  const warmRaven = e => {
    if (!motion?.enabled || !e.target.closest('[data-artist="raven-lin"]')) return;
  ...
  grid.addEventListener('click', e => {
    if (e.target.closest('[data-raven-replay]')) { effect(true); return; }
  ...
  document.addEventListener('club:motion', () => {
    const replay = grid.querySelector('[data-raven-replay]');
    if (replay) replay.disabled = motion?.enabled === false;
  });
